...
def test_postgresql_ddl_package_has_required_files_and_syntax(tmp_path):
    ...
    assert 'CREATE SCHEMA IF NOT EXISTS "errdain_ecommerce";' in sql["ddl/schema.sql"]
    assert "BOOLEAN" in sql["ddl/tables.sql"]
    assert "TIMESTAMP" in sql["ddl/tables.sql"]
    assert 'CREATE TABLE "errdain_ecommerce"."marketplace_customers"' in sql["ddl/tables.sql"]


def test_mssql_ddl_package_has_required_files_and_syntax(tmp_path):
    sql = _package_sql(tmp_path, "education", "mssql")
    assert "CREATE SCHEMA [errdain_education]" in sql["ddl/schema.sql"]
...
def test_mysql_ddl_package_has_required_files_and_syntax(tmp_path):
    ...
    assert "CREATE DATABASE IF NOT EXISTS `errdain_telecommunications`;" in sql["ddl/schema.sql"]
    assert "BOOLEAN" in sql["ddl/tables.sql"]
    assert "DATETIME" in sql["ddl/tables.sql"]
    assert "USE `errdain_telecommunications`;" in sql["ddl/schema.sql"]
...
def test_foreign_key_sql_preserves_representative_domain_relationships(tmp_path):
    expectations = {
        "retail": ['FOREIGN KEY ("category_id") REFERENCES "errdain_retail"."categories" ("category_id")'],
        "manufacturing": ['FOREIGN KEY ("factory_id") REFERENCES "errdain_manufacturing"."factories" ("factory_id")'],
        "telecommunications": ['FOREIGN KEY ("subscription_id") REFERENCES "errdain_telecommunications"."subscriptions" ("subscription_id")'],
        "education": ['FOREIGN KEY ("institution_id") REFERENCES "errdain_education"."institutions" ("institution_id")'],
        "ecommerce": ['FOREIGN KEY ("customer_id") REFERENCES "errdain_ecommerce"."marketplace_customers" ("customer_id")'],
    }
