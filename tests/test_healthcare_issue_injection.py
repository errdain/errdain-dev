import json

import pytest

from errdain.cli import main
from errdain.domains.healthcare.generators import HealthcareGenerator
from errdain.domains.healthcare.schemas import HEALTHCARE_SPEC
from errdain.injector import FailureInjector
from errdain.validation import validate


def test_healthcare_issue_injection_uses_shared_engine_for_selected_tables():
    clean = HealthcareGenerator(80, seed=39).generate()
    rates = {
        "null_values": 0.02,
        "duplicate_records": 0.02,
        "datatype_mismatch": 0.02,
        "foreign_key_break": 0.02,
        "invalid_dates": 0.02,
        "negative_values": 0.02,
        "outliers": 0.02,
        "missing_records": 0.01,
    }
    injected, events = FailureInjector(rates, seed=39, spec=HEALTHCARE_SPEC).apply(clean, {"claims", "payments"})
    assert {event.table for event in events} <= {"claims", "payments"}
    assert sum(event.count for event in events) > 0
    assert validate(injected, HEALTHCARE_SPEC)["overall_status"] == "FAIL"


def test_healthcare_integration_generate_inject_validate_export_csv_json_parquet(tmp_path):
    pytest.importorskip("pyarrow")
    assert main([
        "--domain", "healthcare",
        "--records", "50",
        "--tables", "claims", "payments",
        "--inject-failures", "true",
        "--failure-profile", "low",
        "--output-format", "csv", "json", "parquet",
        "--output", str(tmp_path),
        "--dataset-name", "healthcare-integration",
    ]) == 0
    run = next((tmp_path / "healthcare-integration").iterdir())
    assert (run / "bulk" / "claims.csv").exists()
    assert (run / "bulk" / "claims.json").exists()
    assert (run / "bulk" / "claims.parquet").exists()
    assert (run / "bulk" / "payments.csv").exists()
    assert (run / "bulk" / "payments.json").exists()
    assert (run / "bulk" / "payments.parquet").exists()
    failures = json.loads((run / "failure_report.json").read_text())
    assert failures["total_injected"] > 0
